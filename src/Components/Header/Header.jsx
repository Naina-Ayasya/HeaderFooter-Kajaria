...
import { FiSearch, FiX, FiUser, FiMapPin, FiMenu, FiCamera } from "react-icons/fi";
...
const Header = () => {
 ...
 const toggleDropdown = (menu) => { 
    if (openDropdown === menu) {
      ...
      }, 700); // Match this duration with your CSS animation duration
    } else {
  ...
  return (
    <div ref={headerRef} className="relative font-urbanist xl:mt-6 right-[2px] w-screen bg-white z-[100]">
      {/* Top Contact Bar */}
      ...
      {/* Main Header */}
      <div className="mt-0">
        <div className="flex items-center w-full px-6 py-7 shadow">
          <div className="flex justify-center xl:justify-start w-full">
            <img src={logo} alt="logo" className="md:ml-12 xl:ml-32 w-36"/>
          </div>
...
<div className="flex items-center justify-end xl:hidden">
  ...
  {mobileMenuOpen ? (
    ...
    <FiMenu className="w-[24px] h-[24px] absolute left-2 top-[40px] ml-5 xl:hidden" />
          ...
          <nav className="hidden xl:flex absolute left-[370px]  mr-[250px] ml-12 gap-8 font-semibold text-nowrap text-xs">
          ...
          <div className="hidden xl:flex gap-10 mr-32 items-center ">
            ...
            <img
              src={profile}
              alt='profile'
              className="size-5 mr-12 cursor-pointer"
              onClick={() => setIsDropdownOpen(!isDropdownOpen)}
            />
            <img src={dropdown} alt="drop" className="-ml-24"/>
            {/* Profile Dropdown */}
            {isDropdownOpen && (
        ...
        {openDropdown === 'PRODUCTS' && (
          <div className={`absolute top-[100%] xl:w-screen h-[418px] bg-white shadow py-4 grid grid-cols-7 gap-12 text-sm ${animateOut ? 'slide-fade-up' : 'slide-fade-down'}`}>
            <div>
              <h3 className="font-urbanist text-3xl pl-[70px] mb-8">Categories</h3>
              <ul className="space-y-6 ">
                <li className="flex flex-col-2 -m-1 ml-20 hover:text-blue-700 text-nowrap">Glazed Vitrified Tiles -<img src={eternity} alt="eternity" className="w-[85.26px] h-[12px] ml-1 my-1"/></li>
                <li className="flex flex-col-2 -m-1 ml-20 hover:text-blue-700 text-nowrap">Gres Floor & Wall Tiles - <img src={gress} alt="gress" className="w-[45.5px] h-[14px] ml-1 my-0"/></li>
                <li className="flex flex-col-2 -m-1 ml-20 hover:text-blue-700 text-nowrap">Polished Vitrified Tiles - <img src={vitronite} alt="vitronite" className="w-[45.5px] h-[14px] ml-1 my-0"/></li>
                <li className="flex flex-col-2 -m-1 ml-20 hover:text-blue-700 text-nowrap">Faucets and Sanitaryware - <img src={kerovit} alt="kerovit" className="w-[45.5px] h-[14px] ml-1 my-0"/></li>
                <li className="flex flex-col-2 -m-1 ml-20 hover:text-blue-700 text-nowrap">Tile Adhesive - <img src={gressbond} alt="gressbond" className="w-[96.59px] h-[15px] ml-1 my-0"/></li>
              </ul>
            </div>
            <img src={line} alt="line" className="h-[417px] mx-44 text-gray-600"/>
            ...
            <img src={line} alt="line" className="h-[417px] mx-[40px] text-gray-600"/>
            <div className="flex flex-col justify-center items-end mb-28">
            ...
            <img src={menutiles} alt="tile" className="w-full h-[380px]"/>
            <img src={tile} alt="image" className="max-w-96 h-[426px] opacity-100 -mx-[1220px]"/>
          </div>
        )}
 
        {openDropdown === 'ABOUT US' && (
          <div className={`absolute top-[100%] left-[350px] w-[430px] h-[438px] bg-white px-20 py-4 shadow text-sm ${animateOut ? 'slide-fade-up' : 'slide-fade-down'}`}>
        ...
        {openDropdown === 'RESOURCES' && (
          <div className={`absolute top-[100%] left-[600px] w-[230px] h-[231px] bg-white px-20 py-4 shadow text-sm ${animateOut ? 'slide-fade-up' : 'slide-fade-down'}`}>
        ...
        {openDropdown === 'INVESTOR' && (
          <div className={` absolute top-[100%] w-[1512px] h-[418px] bg-white py-4 shadow grid grid-cols-8 gap-10 text-sm ${animateOut ? 'slide-fade-up' : 'slide-fade-down'}`}>
      ...
      {isOpen && (
        <div className=" bg-black bg-opacity-30 z-50 flex items-center justify-center">
          <div className="bg-white w-[1512px] h-[726px] p-6 shadow-lg relative">
             <FiX
    className="absolute bottom-[763px] right-[190px] size-5 text-yellow-600 cursor-pointer"
    onClick={() => setIsOpen(false)}
  />
  ...
  <div className="ml-[120px] max-w-[1230px]">
    <div className="py-6">
    <p className="absolute left-[150px] text-xs text-gray-500 ">RECENT SEARCHES</p>
    </div>
    <ul className=" text-sm">
                {['Ceramic Tiles', 'Wall Tiles', 'Floor Tiles'].map((item, i) => (
                  <li key={i} className="py-3 border-b border-gray-300 flex mt-4 items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                    <img src={history} alt="history" className="w-[24px] h-[24px]"/> {item}
                    </div>
              ...
              <div className="ml-[120px] max-w-[1230px] space-y-10">
                <div className="py-6">
    <p className="absolute text-xs text-gray-500 mb-2">POPULAR SEARCHES</p>
    </div>
    <ul className="text-sm space-y-10">
                {['Porcelain Tiles', 'Kitchen Tiles', 'Bathroom Tiles'].map((item, i) => (
                  <li key={i} className=" border-b border-gray-300 flex mt-4 items-center justify-between gap-2">
                    {item}
